...
fetch("/scrape-sites").then(response => response.json()).then(data => { stopCount(); sortCategories(data) })
    ...
    for (let key in data) {
        switch (key) {
            case "dealsod":
                ...
                dealsOA = createDisplay(data.dealsod, "Deals of America", "dealsofamerica")
                break;
